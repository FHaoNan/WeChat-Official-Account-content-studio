...
- P21 后证据句从“有 `[Sx]`”升级为“证据强度可解释”：`generated/evidence-report.json` 会输出 `claim_audits`，每条带 `claim`、`source_ids`、`support_level`（direct/indirect/background）、`support_reason`、`needs_human_review`；summary 汇总 support_levels 和需要人工复核的句子数。`needs_human_review` 是编辑提示，不等同 quality warning；未加引用的关键判断仍 fail closed。
- `output/` 目录是默认新稿目录。
...
如果要从热点文件直接产出可检查草稿，运行 P5/P6 总控命令：`python3 {skill_dir}/toolkit/cli.py auto-draft --hotspots {hotspots_json} --limit 1 --author "烧 Token 的人"`；如果要自动修订一轮，追加 `--revise-once`。它会串联 `select_ai_topics.py` → `research_sources.py` → `draft_writer.py` → `draft-from-topic`，并落盘 `_auto-draft/<hotspots_stem>/selected-topics.json`、`topic-with-sources.json`、`sources.json`、`research-report.json`，最后输出 `article_dir` 和所有下游 artifacts。`--revise-once` 会运行 `scripts/revise_draft.py`，根据 `evidence-report.json` 给无引用判断补 `[Sx]`，把缺失来源类别写入 `generated/revision-report.json` 的 blockers，然后重新 render 与 quality gates。`draft_writer.py` 会把 sources 的 snippet/summary/description 转为带 `[S1]` 来源标注的公众号正文 v1，并写入 `generated/evidence-ledger.json`；它不会编造来源或事实，没有摘录的来源只标记为待深读。默认 source research 找不到一手/primary 来源会 fail closed；社区讨论和英文媒体是可选补充，不再硬凑齐三类。仅探索时加 `--allow-incomplete-sources`。测试/离线可加 `--search-fixture fixture.json`，禁止联网可加 `--no-network`。

选题已经补齐可验证 URL 来源后，可一键串联生成草稿和报告：`python3 {skill_dir}/toolkit/cli.py draft-from-topic --topic-file {topic_json} --author "烧 Token 的人"`。该命令会创建标准文章目录，写入 `article.md`、`generated/sources.json`、`generated/topic.json`、`generated/evidence-ledger.json`、`generated/evidence-report.json`，并自动执行 render 与 quality gates（含 source gate 和 evidence gate）。如果希望质量门禁非 pass 时直接返回非 0，加 `--strict-check`。注意：该命令不会编造 URL；source gate 现在要求至少有一手/primary 来源，社区讨论和英文媒体是可选支撑信号；正文可以只保留 `[Sx]` 内部证据锚点，不要求把 URL 链接展示在发布正文里。没有可验证一手来源时 `source_credibility` 会 fail，正文关键判断没有 `[Sx]` 引用时 `evidence_coverage` 会 fail，必须先补证或补引用。
...
- 自动模式 → 选最高分；如果分数接近，优先选国内热度更高、跨平台出现、且自然符合 AI/模型/产品方向的话题
- 交互模式 → 展示 10 个，等用户选
